class Admin::UsersController < ApplicationController
  ...
  def index
    redirect_to menus_path and return if current_user.access_type != User::ACCESS_TYPE::KANRISHA
    @users = User.order(access_type: :desc)
  ...
  def update
    ...
    begin
      ActiveRecord::Base.transaction() do
        if @user.save
          redirect_to admin_users_path, flash: {success: t('message.success_completed')}
        else
          render 'edit'
        end
      end
  ...
  def update_profile
    ...
    begin
      ActiveRecord::Base.transaction() do
        if @user.save
          redirect_to admin_profile_user_path, flash: {success: t('message.success_completed')}
        else
          render 'profile'
        end
      end
